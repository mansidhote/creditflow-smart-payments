...
export default function Invoices() {
  ...
  const markAsPaid = async (invoice: Invoice) => {
    ...
    if (profile.cash_balance < amountPaid) {
      toast.error(
        `Insufficient balance. You have ${formatINR(profile.cash_balance)} but need ${formatINR(amountPaid)}.`,
      );
      return;
    }
    ...
    await supabase
      .from("profiles")
      .update({
        cash_balance: profile.cash_balance - amountPaid,
      })
      .eq("user_id", user.id);
    ...
    toast.success(
      `Paid ${formatINR(amountPaid)}` +
        (discountCaptured > 0
          ? ` — saved ${formatINR(discountCaptured)}! 🎉`
          : ""),
    );
  };
  ...
  const filtered =
    tab === "all"
      ? invoices
      : invoices.filter((i) => i.status === tab.toUpperCase());
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                      ...
                      {inv.discount_deadline && discountDays !== null && (
                        <span
                          className={
                            discountDays > 0 ? "text-success" : "text-critical"
                          }
                        >
                          Discount:{" "}
                          {discountDays > 0
