...
import { parseCreditTerm, calcEAC, formatINR, getDaysLeft, getDaysLeftColor, getDaysLeftLabel, COMMON_TERMS } from '@/lib/creditTerms';
...
export default function Invoices() {
  ...
  const fetchData = async () => {
    ...
    const [{ data: inv }, { data: sup }] = await Promise.all([
      supabase.from('invoices').select('*, suppliers(name)').eq('user_id', user.id).order('due_date', { ascending: true }),
  ...
  const handleCreate = async () => {
    if (!user || !form.supplier_id || !form.amount) { toast.error('Please fill required fields'); return; }
    ...
    const { error } = await supabase.from('invoices').insert({
      user_id: user.id, supplier_id: form.supplier_id, amount: parseFloat(form.amount),
    ...
    // Auto-insert alerts
    const invRes = await supabase.from('invoices').select('id').eq('user_id', user.id).order('created_at', { ascending: false }).limit(1).single();
    if (invRes.data) {
      const alertRows: any[] = [];
      ...
      [7, 3, 0].forEach(daysBefore => {
        ...
        const type = daysBefore === 7 ? 'DUE_7_DAYS' : daysBefore === 3 ? 'DUE_3_DAYS' : 'DUE_TODAY';
        alertRows.push({ user_id: user.id, invoice_id: invRes.data.id, type, trigger_at: triggerAt.toISOString() });
      });
      if (discountDeadline) {
        const dd = new Date(discountDeadline);
        dd.setDate(dd.getDate() - 1);
        alertRows.push({ user_id: user.id, invoice_id: invRes.data.id, type: 'DISCOUNT_EXPIRING' as const, trigger_at: dd.toISOString() });
      }
    ...
    <div className="space-y-6">
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="space-y-2">
                ...
                <Select value={form.terms} onValueChange={v => setForm(f => ({ ...f, terms: v }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {COMMON_TERMS.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
                    <SelectItem value="Custom">Custom...</SelectItem>
                  </SelectContent>
                </Select>
                {form.terms === 'Custom' && (
                  <Input value={form.custom_terms} onChange={e => setForm(f => ({ ...f, custom_terms: e.target.value }))} placeholder="e.g. 2/10 Net 30" className="mt-2" />
                )}
              </div>
              <div className="space-y-2">
      ...
      {filtered.length === 0 ? (
        <div className="bg-card border border-border rounded-lg p-12 text-center">
          ...
          <h3 className="font-heading font-bold text-lg mb-1">No invoices found</h3>
          <p className="text-sm text-muted-foreground">
            {tab === 'all' ? 'Create your first invoice to start tracking payments' : `No ${tab.replace('_', ' ')} invoices`}
          </p>
        </div>
      ) : (
              ...
              <div key={inv.id} className="bg-card border border-border rounded-lg p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`font-mono font-medium text-sm ${getDaysLeftColor(days)}`}>
                      {getDaysLeftLabel(days)}
                    </span>
                    {inv.status !== 'PAID' && (
                      <Button size="sm" onClick={() => markAsPaid(inv)}>Mark Paid</Button>
                    )}
                  </div>
                </div>
              </div>
